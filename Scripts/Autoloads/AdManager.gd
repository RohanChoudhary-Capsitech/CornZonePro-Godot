...
func connect_signals():
	...
	if admob.has_signal("interstitial_ad_showed_full_screen_content"):
	...
	if admob.has_signal("rewarded_interstitial_ad_loaded"):
		admob.rewarded_interstitial_ad_loaded.connect(_on_rewarded_interstitial_loaded)

	if admob.has_signal("rewarded_interstitial_ad_failed_to_load"):
		admob.rewarded_interstitial_ad_failed_to_load.connect(_on_rewarded_interstitial_failed)

	if admob.has_signal("rewarded_interstitial_ad_showed_full_screen_content"):
		admob.rewarded_interstitial_ad_showed_full_screen_content.connect(_on_ad_opened)

	if admob.has_signal("rewarded_interstitial_ad_dismissed_full_screen_content"):
		admob.rewarded_interstitial_ad_dismissed_full_screen_content.connect(_on_rewarded_interstitial_closed)

	# =====================================================
	# REWARD
	# =====================================================
	if admob.has_signal("rewarded_ad_user_earned_reward"):
		admob.rewarded_ad_user_earned_reward.connect(_on_user_earned_reward)

	if admob.has_signal("rewarded_interstitial_ad_user_earned_reward"):
		admob.rewarded_interstitial_ad_user_earned_reward.connect(_on_user_earned_reward)
...
func load_rewarded_interstitial():
	...
	if rewarded_interstitial_retries >= max_retries:
		#log_msg("Rewarded Interstitial Max Retries Reached")
		return
...
func show_rewarded_interstitial(callback = null):
	pending_reward_callback = callback
	if rewarded_interstitial_loaded:
		#log_msg("Showing Rewarded Interstitial")
		admob.show_rewarded_interstitial_ad()
	else:
		#log_msg("Rewarded Interstitial Not Ready")
		load_rewarded_interstitial()


func _on_rewarded_interstitial_loaded(ad_info, response_info):
	#log_msg("Rewarded Interstitial Loaded")
	rewarded_interstitial_loaded = true
	rewarded_interstitial_retries = 0

func _on_rewarded_interstitial_failed(ad_info, error):
	#log_msg("Rewarded Interstitial Failed")
	#print(error.get_message())
	await get_tree().create_timer(retry_time).timeout
	load_rewarded_interstitial()
...
# REWARD EARNED
# =========================================================
func _on_user_earned_reward(ad_info, reward_data):
