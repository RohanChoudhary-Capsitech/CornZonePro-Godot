...
const PAGE_SIZE = 10
const LEADERBOARD_SYNC_INTERVAL_SECONDS = 300
const CACHE_TTL_SECONDS = LEADERBOARD_SYNC_INTERVAL_SECONDS
...
var _entry_ensured_for_uid: String = ""
var _sync_in_progress: bool = false
 
func _ready():
	_ensure_collection()
	var sync_timer := Timer.new()
	sync_timer.wait_time = LEADERBOARD_SYNC_INTERVAL_SECONDS
	sync_timer.one_shot = false
	sync_timer.autostart = true
	add_child(sync_timer)
	sync_timer.timeout.connect(_sync_leaderboard_periodically)
...
func update_entry():
	_ensure_collection()
	await _enterDataFirebase()
 
 
func _sync_leaderboard_periodically() -> void:
	if _sync_in_progress:
		return
	if FirebaseManager.player_id == "" or not PlayerData.has_loaded_data:
		return
	if not await FirebaseManager.internet_available():
		return
 
	_sync_in_progress = true
	await update_entry()
	await fetch_top(true)
	_sync_in_progress = false
 
 
func ensure_current_player_entry(force_write: bool = false) -> bool:
	_ensure_collection()
	if FirebaseManager.player_id == "":
		return false
 
	var uid = FirebaseManager.player_id
	if not force_write and _entry_ensured_for_uid == uid:
		return true
 
	var doc = await _col.get_doc(uid)
	var data := {}
	if doc != null and doc.has_method("get_unsafe_document"):
		data = doc.get_unsafe_document()
	FirebaseManager.print_firebase_read(COLLECTION, uid, data, doc != null and not data.is_empty())
 
	if force_write or doc == null or data.is_empty():
		await _enterDataFirebase()
 
	_entry_ensured_for_uid = uid
	return true
...
func fetch_top(force_refresh: bool = false) -> Array:
	_ensure_collection()
	await ensure_current_player_entry(false)
	...
	if my_rank <= 0:
		my_rank = _get_my_rank_from_entries(entries)
	if my_rank <= 0 and FirebaseManager.player_id != "":
		my_rank = entries.size() + 1
